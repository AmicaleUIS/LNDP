-- ============================================================
-- LE NID DES PRONOS — PATCH V2.0.1
-- Accès au carnet PDF collector après la compétition.
--
-- Règles :
--   * chaque joueur authentifié peut ouvrir uniquement son propre carnet ;
--   * le super admin peut ouvrir et exporter tous les carnets ;
--   * le script remplace l’ancienne fonction qui pouvait encore être limitée
--     au super admin sur certaines installations.
--
-- Ce patch est rejouable.
-- ============================================================
...
begin
  if auth.uid() is distinct from p_user_id and not public.is_super_admin() then
    raise exception 'Tu peux uniquement ouvrir ton propre carnet PDF.';
  end if;
  ...
  -- Le journal admin refuse volontairement les joueurs ordinaires.
  -- On ne journalise donc l’ouverture que lorsqu’elle est effectuée par le super admin.
  if public.is_super_admin()
     and to_regprocedure('public.admin_log_action(text,text,jsonb,jsonb)') is not null then
