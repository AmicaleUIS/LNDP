...
begin
  ...
  if not public.is_second_winner_candidate(new.competition_id, new.predicted_team_id) then
    raise exception 'Cette équipe n’est pas qualifiée pour le 2e choix champion.';
  end if;
...
begin
  ...
  if target_competition_id is null then
    select id into target_competition_id
  ...
  set predicted_team_id = excluded.predicted_team_id,
      updated_at = now()
  returning swp.user_id, swp.competition_id, swp.predicted_team_id, swp.created_at, swp.updated_at;
end;
$$;
...
first_winner_points as (
  select
    user_id,
    coalesce(sum(points_total), 0)::int as first_winner_points,
...
second_winner_points as (
  select
    user_id,
    coalesce(sum(points_total), 0)::int as second_winner_points,
    (array_agg(predicted_team_id) filter (where points_total = 50))[1] as second_winner_team_id,
    max(predicted_team_name) filter (where points_total = 50) as second_winner_team_name
  from public.v_second_winner_predictions
  group by user_id
),
base as (
  select
