// ============================================================
// LE NID DES PRONOS — ADMIN V1.9.2
// ============================================================
...
const Admin = {
  ...
  async loadOwlPollResultsAdmin() {
    ...
    if (error) {
      console.warn("Résultats de sondage Hibou indisponibles : lance le patch SQL V1.9.2", error);
      this.state.owlPollResults = [];
      return;
    }
...
Je m’en remets au Hibou">${H.escapeHtml(this.owlPollOptionsText(msg))}</textarea><small class="muted">Un choix par ligne. Tu peux faire Oui/Non, 3 choix, 4 choix, etc. Maximum 8 choix.</small></label>
  ...
  async saveOwlLoginMessage(form) {
    ...
    if (error) {
      H.toast(error.message || "Impossible d’enregistrer le message. Lance le patch SQL V1.9.2.", "error");
      return;
    }
  ...
  async saveChampionBonusPoints() {
    ...
    if (error) {
      H.toast(error.message || "Impossible d’enregistrer les points bonus. Lance le patch SQL V1.9.2.", "error");
      return;
    }
  ...
  async cleanStartPreservePredictions() {
    ...
    if (error) {
      H.toast(error.message || "Reset classements impossible. Lance le patch SQL V1.9.2.", "error");
      return;
    }
